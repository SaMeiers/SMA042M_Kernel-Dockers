...
/*hs04 code for BSPAL6398A-19 by shixuanxuan at 20221001 start*/
#ifdef CONFIG_HQ_PROJECT_HS04
		if(strstr(req->buf, "SWATD")) {
			pr_err("%s: tx at cmd %s\n", __func__, req->buf);
		}

		if(strstr(req->buf, "ACTIVATE")) {
			pr_err("%s: tx at cmd %s\n", __func__, req->buf);
                }

		if(strstr(req->buf, "MODECHAN")) {
			pr_err("%s: tx at cmd %s\n", __func__, req->buf);
                }
#endif
/*hs04 code for BSPAL6398A-19 by shixuanxuan at 20221001 end*/
